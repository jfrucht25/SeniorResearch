...
for i in range(1000):
	if random.uniform(0,1) < probabilityWE:
		xml_file += '    <vehicle id="we_%i" type="GenericCar" route="west_east" depart="%i" />' % (vehNr, i)
		vehNr += 1
	if random.uniform(0,1) < probabilityEW:
		xml_file += '    <vehicle id="ew_%i" type="GenericCar" route="east_west" depart="%i" />' % (vehNr, i)
		vehNr += 1
	if random.uniform(0,1) < probabilityNS:
		xml_file += '    <vehicle id="ns_%i" type="GenericCar" route="west_east" depart="%i" />' % (vehNr, i)
		vehNr += 1
	if random.uniform(0,1) < probabilitySN:
		xml_file += '    <vehicle id="sn_%i" type="GenericCar" route="west_east" depart="%i" />' % (vehNr, i)
		vehNr += 1
