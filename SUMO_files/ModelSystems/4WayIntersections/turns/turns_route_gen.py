...
with open("turns.rou.xml","w") as f:
	f.write(xml_file)
'''
import sumolib, sys
fileName = 'turns.net.xml'
if len(sys.argv) > 1:
    fileName = sys.argv[1]
net = sumolib.net.readNet(fileName)
print(net.getEdge("EW_1").getToNode().getID())
print(net)
print(i for i in sumolib.xml.parse_fast(fileName, 'edge', ['id', 'value']))
for edge in sumolib.xml.parse_fast(fileName, 'edge', ['id', 'value']):
    print(edge)
    print(edge.value)